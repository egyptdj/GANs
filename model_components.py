            ...
            with tf.name_scope(model_scope):
                with tf.variable_scope(self.scope, reuse=reuse):
                    ...
                    if self.type=='gan':
                        ...
                        _z = tf.layers.conv2d_transpose(inputs=_z, filters=_x[2], kernel_size=[4,4], kernel_initializer=tf.initializers.truncated_normal(stddev=0.02), strides=2, padding='SAME', name='deconv1')
