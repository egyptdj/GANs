            ...
            with tf.name_scope(model_scope):
                with tf.variable_scope(self.scope, reuse=reuse):
                    ...
                    if self.type=='gan':
                        ...
                        _z = tf.layers.batch_normalization(inputs=_z, training=self.training, name='batchnorm1')
                        _z = tf.nn.relu(_z, name='relu1')
                        _z = tf.layers.conv2d_transpose(inputs=_z, filters=128, kernel_size=[5,5], kernel_initializer=self.initializer, kernel_regularizer=self.regularizer, kernel_constraint=self.constraint, strides=2, padding='SAME', name='deconv2')
                        _z = tf.layers.batch_normalization(inputs=_z, training=self.training, name='batchnorm2')
                        _z = tf.nn.relu(_z, name='relu2')
                        _z = tf.layers.conv2d_transpose(inputs=_z, filters=64, kernel_size=[5,5], kernel_initializer=self.initializer, kernel_regularizer=self.regularizer, kernel_constraint=self.constraint, strides=2, padding='SAME', name='deconv3')
                        _z = tf.layers.batch_normalization(inputs=_z, training=self.training, name='batchnorm3')
                        _z = tf.nn.relu(_z, name='relu3')
                        _z = tf.layers.conv2d_transpose(inputs=_z, filters=_x[2], kernel_size=[5,5], kernel_initializer=self.initializer, kernel_regularizer=self.regularizer, kernel_constraint=self.constraint, strides=2, padding='SAME', name='deconv4')
